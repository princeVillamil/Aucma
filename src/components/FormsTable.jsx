...
export default function FormsTable({
  ...
  const paginatedRequests = isPrinting
    ? sortedRequests
      ...
      >
        <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-900">
          <tr>
            ...
            {!isPrinting && <th className="px-4 py-3 text-right">Actions</th>}
          </tr>
        </thead>
        ...
        <tbody>
          {paginatedRequests.map((req, idx) => (
            <tr key={idx} className="border-t border-gray-200">
              <td
                className="px-4 py-4 font-medium text-gray-900 cursor-pointer hover:underline"
              ...
              <td className="px-4 py-4">
                {req.technician == null ? "Pending" : req.technician}
              </td>
              <td className="px-4 py-4">
                {req.preferredDate} <br />{" "}
                {convertTo12HourFormat(req.preferredTime)}
              </td>
              <td className="px-4 py-4">
                <span
      ...
      {!isPrinting && (
        <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
          <span>
            ...
            -{Math.min(currentPage * itemsPerPage, sortedRequests.length)} of{" "}
            {sortedRequests.length}
          </span>
